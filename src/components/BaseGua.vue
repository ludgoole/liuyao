...
<template>
  <div class="BaseGua flex flex-col-reverse">
    <div v-for="(yao, index) in guaxiang" :key="index" class="item flex items-center" @click="emit('on-change', 纳甲[index])">
      ...
      <div class="BaseGua-right text-left">
        ...
        <div flex>
          <p w-10 text-2xl color-gray-6>
            <span
              v-if="hasDongyao" inline-block :class="{
                'color-black': yao % 3 === 0,
                'translate-y--10px': yao % 3 !== 0,
                'translate-y--4px': yao === 9,
              }"
            >
              {{ yao === 6 ? '×' : yao === 9 ? 'o' : yao === 7 ? '、' : '、、' }}
            </span>
            <span v-else inline-block translate-y--10px>
              {{ yao % 2 === 0 ? '、' : '、、' }}
            </span>
          </p>
          <p v-if="hasShiying" translate-y-6px>
            {{ 世应[`${index + 1}`] === '世' ? 卦?.八宫.slice(0, 1) : 世应[`${index + 1}`] }}
          </p>
          <p v-if="!hasDongyao" ml-4 translate-y-6px>
            {{ 六神[index] }}
          </p>
        </div>
      </div>
